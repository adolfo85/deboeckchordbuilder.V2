...
const App: React.FC = () => {
    ...
    const handleToggleNote = useCallback((stringIndex: number, fret: number) => {
        ...
        if (setRootMode) {
            ...
            if (exists) setRootNote(exists);
            else { const others = selectedNotes.filter(n => n.stringIndex !== stringIndex); setSelectedNotes([...others, newNote]); setRootNote(newNote); }
            setSetRootMode(false); return;
        }
        const exists = selectedNotes.find(n => n.stringIndex === stringIndex && n.fretNumber === fret);
        if (exists) {
            const remaining = selectedNotes.filter(n => n !== exists); setSelectedNotes(remaining);
            if (rootNote && exists.stringIndex === rootNote.stringIndex && exists.fretNumber === rootNote.fretNumber) setRootNote(null);
        } else {
            const others = selectedNotes.filter(n => n.stringIndex !== stringIndex); const nextNotes = [...others, newNote]; setSelectedNotes(nextNotes);
            if (nextNotes.length === 1 && !rootNote) setRootNote(newNote);
        }
    }, [selectedNotes, rootNote, setSetRootMode, useFlats]);

    const handleNoteClick = (note: NoteData) => {
        if (setRootMode) { setRootNote(note); setSetRootMode(false); return; }
    ...
    const handleGraphicDragStart = (e: React.MouseEvent, id: string) => {
        e.stopPropagation(); setSelectedGraphicId(id); setDocToolMode('graphics');
        const graphic = graphics.find(g => g.id === id); if (!graphic) return;
        const startX = e.clientX; const startY = e.clientY; const initX = graphic.x; const initY = graphic.y;
        const onMouseMove = (ev: MouseEvent) => { updateGraphic(id, 'x', initX + (ev.clientX - startX)); updateGraphic(id, 'y', initY + (ev.clientY - startY)); };
        const onMouseUp = () => { window.removeEventListener('mousemove', onMouseMove); window.removeEventListener('mouseup', onMouseUp); };
        window.addEventListener('mousemove', onMouseMove); window.addEventListener('mouseup', onMouseUp);
    };
    const handleGraphicResizeStart = (e: React.MouseEvent, id: string, handle: 'w' | 'h' | 'wh') => {
        e.stopPropagation(); const graphic = graphics.find(g => g.id === id); if (!graphic) return;
        const startX = e.clientX; const startY = e.clientY; const startW = graphic.width; const startH = graphic.height;
        const onMouseMove = (ev: MouseEvent) => {
            const dx = ev.clientX - startX; const dy = ev.clientY - startY;
            const rad = graphic.rotation * (Math.PI / 180);
            const localDx = dx * Math.cos(rad) + dy * Math.sin(rad); const localDy = -dx * Math.sin(rad) + dy * Math.cos(rad);
            if (handle === 'w' || handle === 'wh') updateGraphic(id, 'width', Math.max(20, startW + localDx));
            if (handle === 'h' || handle === 'wh') updateGraphic(id, 'height', Math.max(20, startH + localDy));
        };
        const onMouseUp = () => { window.removeEventListener('mousemove', onMouseMove); window.removeEventListener('mouseup', onMouseUp); };
        window.addEventListener('mousemove', onMouseMove); window.addEventListener('mouseup', onMouseUp);
    };
    ...
    const getFontFamily = (font: string | undefined) => {
        ...
        if (font === 'Serif') return "serif"; if (font === 'Mono') return "monospace"; return "sans-serif";
    }
    ...
    return (
        <div className="h-screen w-full flex flex-col bg-slate-950 text-white overflow-hidden relative font-sans">
            ...
            {/* CONTENT */}
            <div className="flex-grow relative overflow-hidden bg-slate-900 flex flex-col">
                ...
                <div className={`absolute inset-0 flex flex-col transition-transform duration-300 ${viewMode === 'editor' ? 'translate-x-0' : '-translate-x-full'}`}>
                    <div className="flex-grow relative flex flex-col items-center justify-center overflow-hidden" ref={fretboardContainerRef}>
                        <div className="z-40 mb-4"><div className="bg-[#1e293b] border-2 border-slate-600 rounded-xl px-8 py-3 shadow-2xl flex flex-col items-center min-w-[200px]"><span className="text-[10px] uppercase font-bold mb-1 text-slate-400">Acorde</span><div className="text-3xl font-black text-white tracking-tight">{detectedChord || <span className="text-slate-600">---</span>}</div></div></div>
                        ...
                        <div className="flex-grow p-4 overflow-y-auto bg-slate-900 text-slate-300">
                            {activeTab === 'editor' ? (
                                <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 max-w-5xl">
                                    <div className="space-y-3"><label className="text-xs font-bold text-slate-500 uppercase">Etiquetas</label><div className="flex gap-2"><button onClick={() => setNoteDisplayMode('intervals')} className={`px-4 py-2 rounded-lg text-sm font-bold border ${noteDisplayMode === 'intervals' ? 'bg-indigo-600' : 'bg-slate-800'}`}>Intervalos</button><button onClick={() => setNoteDisplayMode('notes')} className={`px-4 py-2 rounded-lg text-sm font-bold border ${noteDisplayMode === 'notes' ? 'bg-indigo-600' : 'bg-slate-800'}`}>Notas</button></div>
                                        <div className="flex gap-2 mt-2"><button onClick={() => setUseFlats(!useFlats)} className={`px-4 py-2 rounded-lg text-sm font-bold border flex items-center gap-2 ${useFlats ? 'bg-indigo-600 border-indigo-500' : 'bg-slate-800 border-slate-700'}`}>{useFlats ? 'Bemoles (b)' : 'Sostenidos (#)'}</button></div></div>
                                    <div className="space-y-3"><label className="text-xs font-bold text-slate-500 uppercase">Fundamental</label><button onClick={() => setSetRootMode(!setRootMode)} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold border ${setRootMode ? 'bg-amber-600 animate-pulse' : 'bg-slate-800'}`}><MousePointer2 size={16} /> Definir Fundamental</button></div>
                                    <div className="space-y-3"><label className="text-xs font-bold text-slate-500 uppercase">Herramientas</label><div className="flex gap-2"><button onClick={() => playNotes(selectedNotes)} className="p-2 bg-slate-800 hover:bg-green-600 text-slate-300 hover:text-white rounded-lg border border-slate-700"><Play size={20} fill="currentColor" /></button><button onClick={() => { setSelectedNotes([]); setRootNote(null); }} className="p-2 bg-slate-800 hover:bg-red-900 text-slate-300 hover:text-white rounded-lg border border-slate-700"><Trash2 size={20} /></button></div></div>
                                </div>
                            ) : (
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 max-w-5xl">
                                    <div className="space-y-2"><label className="text-xs font-bold text-slate-500 uppercase">Color</label><div className="flex gap-3"><button onClick={() => setAccentColor('slate')} className="w-8 h-8 rounded-full bg-slate-600"></button><button onClick={() => setAccentColor('blue')} className="w-8 h-8 rounded-full bg-cyan-600"></button><button onClick={() => setAccentColor('gold')} className="w-8 h-8 rounded-full bg-amber-500"></button><button onClick={() => setAccentColor('pink')} className="w-8 h-8 rounded-full bg-pink-500"></button></div></div>
                                    <div className="space-y-2"><label className="text-xs font-bold text-slate-500 uppercase">Elementos</label><div className="flex flex-col gap-2"><button onClick={() => setShowFretNumbers(!showFretNumbers)} className="flex justify-between p-2 bg-slate-800 rounded text-sm">Trastes {showFretNumbers ? <ToggleRight /> : <ToggleLeft />}</button><button onClick={() => setShowInlays(!showInlays)} className="flex justify-between p-2 bg-slate-800 rounded text-sm">Puntos {showInlays ? <ToggleRight /> : <ToggleLeft />}</button></div></div>
                                </div>
                            )}
                        </div>
                    </div>
                                ...
                                <>
                                    ...
                                    {selectedGraphicId ? (
                                        <div className="flex items-center gap-3">
                                            ...
                                            {/* Resize Handles (Allowed for all) */}
                                            <div className="flex items-center gap-2 border-l border-slate-600 pl-2"><div className="flex flex-col"><span className="text-[8px] uppercase text-slate-500">Ancho</span><input type="number" value={selectedGraphic?.width || 0} onChange={(e) => updateGraphic(selectedGraphicId, 'width', parseInt(e.target.value))} className="w-10 bg-slate-900 border border-slate-600 rounded text-[10px] px-1" /></div><div className="flex flex-col"><span className="text-[8px] uppercase text-slate-500">Alto</span><input type="number" value={selectedGraphic?.height || 0} onChange={(e) => updateGraphic(selectedGraphicId, 'height', parseInt(e.target.value))} className="w-10 bg-slate-900 border border-slate-600 rounded text-[10px] px-1" /></div></div>
                    ...
                    <div className="flex-grow overflow-auto bg-slate-800 flex justify-center p-8 relative" onClick={() => { setSelectedChordId(null); setSelectedGraphicId(null); }}>
                        ...
                        <div ref={documentRef} className="bg-white text-black shadow-2xl relative transition-transform origin-top" style={{ width: '210mm', minHeight: '297mm', padding: '20mm', display: 'flex', flexWrap: 'wrap', alignContent: 'flex-start', gap: '10px' }}>
                            ...
                            <div className="w-full border-b-2 border-black pb-4 mb-8 flex justify-between items-end relative z-10 pointer-events-none">
                                <div className="flex flex-col w-full mr-8 pointer-events-auto"><input value={docTitle} onChange={(e) => setDocTitle(e.target.value)} className="text-3xl font-bold font-serif outline-none bg-transparent placeholder-gray-300 w-full text-black" /><input value={docSubtitle} onChange={(e) => setDocSubtitle(e.target.value)} className="text-sm text-gray-500 mt-1 outline-none bg-transparent w-full" /></div>
